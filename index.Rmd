...
# m <- filter(d, !is.na(home_pts)) %>% select(home_team, away_team, margin)
...
for (i in 1:n_sim) {
  ...
  # Add the team strength parameters:
  d_19 <- d_19 %>% 
    left_join(select(beta, team, home_beta = beta_adj), by = c("home_team" = "team")) %>% 
    left_join(select(beta, team, away_beta = beta_adj), by = c("away_team" = "team"))
  
  # Simulate outcomes (6 games)
  d_19 <- d_19 %>% 
  ...
  # attach team strength parameters:
  d_20 <- d_20 %>% 
    left_join(select(beta, team, home_beta = beta_adj), by = c("home_team" = "team")) %>% 
    left_join(select(beta, team, away_beta = beta_adj), by = c("away_team" = "team"))
  
  # Simulate division round outcomes (4 games)
  d_20 <- d_20 %>% 
  ...
  # Simulate conference championship games (2):
  d_21 <- d_21 %>% 
  ...
  # attach strength parameters:
  d_22 <- d_22 %>% 
    left_join(select(beta, team, home_beta = beta_adj), by = c("home_team" = "team")) %>% 
    left_join(select(beta, team, away_beta = beta_adj), by = c("away_team" = "team"))
  
  # Simulate Super Bowl outcome:
  d_22 <- d_22 %>% 
...
saveRDS(seasons, file = paste0("simulation_2k_", substr(Sys.time(), 1, 10), ".rds"))
...
names(picks) <- c("Game", "Points", "Kenny", "Nick", "Paul", "Gavin", "Rish")
