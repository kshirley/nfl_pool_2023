...
<!-- scp 'kes@66.228.42.50:/home/kes/public/kennyshirley.com/public_html/nfl2023/*2023-12-25*'
  ...
  if (file.exists(paste0("simulation_10k_", today, ".rds"))) {
    ...
    for (i in 1:n_sim) {
      ...
      out <- data.frame(game = c(div_names, "AFC_Championship_1", "AFC_Championship_2", 
                                 ...
                                 "Super_Bowl_Champion"), 
                        team = c(filter(st, div_winner == 1) %>% pull(team), 
...
if (n_games_played > previous_games_played) {
  df <- data.frame(Name = c("Ken", "Nick", "Paul", "Gavin", "Rish"), 
                   ...
                   Super_Bowl = e_sb_pts)
  df <- arrange(df, desc(Win_Probability))
...
nick_unique <- c("AFC Conference Championship 1")
paul_unique <- c("AFC Conference Championship 1")



kable(picks, format = "html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  column_spec(3, background = ifelse(picks$Game %in% ken_unique, "yellow", "white")) %>% 
  column_spec(4, background = ifelse(picks$Game %in% nick_unique, "yellow", "white")) %>% 
  column_spec(5, background = ifelse(picks$Game %in% paul_unique, "yellow", "white")) %>% 
  column_spec(6, background = ifelse(picks$Game %in% gavin_unique, "yellow", "white")) %>% 
  column_spec(7, background = ifelse(picks$Game %in% rish_unique, "yellow", "white"))
